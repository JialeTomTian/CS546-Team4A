...
class Execution:
    ...
    def run(self) -> Dict[str, Any]:
        original_results, original_perf = self._execute_code(self.original_program, "original")
        refactored_results, refactored_perf = self._execute_code(self.refactored_program, "refactored")
    ...
    def _execute_code(self, code: str, label: str) -> Tuple[Dict[str, Any], Dict[str, float]]:
        results = {}
    ...
    def _check_correctness(self, original_results: Dict[str, Any], refactored_results: Dict[str, Any]) -> bool:
        return original_results.get("status") == refactored_results.get("status") == "passed"

    def _compare_performance(self, original_perf: Dict[str, float], refactored_perf: Dict[str, float]) -> Dict[str, str]:
        return {
            "runtime": "faster" if refactored_perf["runtime"] < original_perf["runtime"] else "slower",
            "memory": "more efficient" if refactored_perf["memory"] < original_perf["memory"] else "less efficient"
        }
